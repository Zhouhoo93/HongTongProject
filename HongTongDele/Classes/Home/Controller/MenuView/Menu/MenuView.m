...
-(instancetype)initWithDependencyView:(UIView *)dependencyView MenuView:(UIView *)leftmenuView isShowCoverView:(BOOL)isCover{
    ...
    if(self = [super init]){
        ...
        self.coverViewframe = CGRectMake(0, 0, kSCREEN_WIDTH , self.menuViewframe.size.height+20);
    }
    return self;
}
...
-(void)show{
    ...
    self.leftMenuView.frame = CGRectMake(self.bounds.size.width+self.menuViewframe.size.width, self.menuViewframe.origin.y, self.menuViewframe.size.width, self.menuViewframe.size.height);
    self.coverView.frame = CGRectMake(self.bounds.size.width+self.menuViewframe.size.width, self.menuViewframe.origin.y, kSCREEN_WIDTH, self.menuViewframe.size.height);
...
-(CGRect)coverViewframe{
    ...
    if((CGRectEqualToRect(_coverViewframe, CGRectZero)) == YES){
        ...
        _coverViewframe = CGRectMake(0, 0, kSCREEN_WIDTH , self.menuViewframe.size.height);
    }
